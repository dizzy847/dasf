...
class RobotGUI(tk.Tk):
    ...
    def create_control_panel(self, parent):
        ...
        self.speed_scale = ttk.Scale(parent, from_=0.1, to=1.0, value=0.5)
    ...
    def create_status_panel(self, parent):
        ...
        for motor in ['Основание', 'Плечо', 'Локоть', 'Запястье', 'Захват']:
            frame = ttk.Frame(motor_frame)
            frame.pack(fill=tk.X, pady=2)

            ttk.Label(frame, text=motor, width=12).pack(side=tk.LEFT)
    ...
    def setup_threads(self):
        ...
        self.running = True
        self.update_thread = threading.Thread(target=self.update_system_status)
        ...
        while self.running:
            try:
                ...
                for motor, indicators in self.motor_indicators.items():
                    state = self.control_system.motor_states[motor.lower()]
    ...
    # Остальные методы управления...
    # ... (power_on, power_off, emergency_stop и т.д.)
